...
func main() {
	...
	var initMsgs map[graph.RawType]Message
	...
	if PPR { // PPR Testing stuff
		...
		initMap := make(map[graph.RawType]Message)
		for _, v := range interestArray {
			initMap[graph.AsRawType(v)] = Message{INITMASS / float64(len(interestArray))}
		}
		initMsgs = initMap
	}
	...
	g := graph.LaunchGraphExecution[*EdgeProperty, VertexProperty, EdgeProperty, Message, Note](new(PageRank), graphOptions, initMsgs)
