...
func GenerateRandomGraph(n, m int, addProbability Probability, graphInput, expectedOutput string) ([]string, []ShortestPathReport) {
	...
	for i < m {
		event := rand.Intn(100)
		if event >= int((1-addProbability)*100.0) && g.WeightedEdges().Len() < n*(n-1) { // Add edge
		...
		fmt.Println("Added timestamp ", i, " to expected reports")
		reports = append(reports, shortestPathReport)
		i++
	}

	writeToFile(graphInput, commands)
	writeToJson(expectedOutput, reports)
